...
def _load_image(state, cache_key, filename, max_w, max_h):
    key = _dim_key(cache_key)
    if key in state:
        return state[key]
...
def _font_for_button(text, max_w, max_h, start_size=20):
    size = start_size
    while size >= 14:
...
def draw_button(screen, rect, text, font=None, accent=(90, 15, 18), hover_accent=(120, 22, 28)):
    if font is None:
        font = _font_for_button(text, rect.width, rect.height)
...
def _layout_zones(choice_count, is_end):
    if is_end:
...
def _draw_monster_panel(screen, state, story_data, scene, scene_bottom, font_small):
    ...
    if image_file:
        sprite = _load_image(state, f"monster_{monster_id}", image_file, panel_w - 24, 140)
...
def _draw_player_name(screen, username, center_x, bottom_y):
    if not username:
        return
...
def _draw_player(screen, state, scene, game_state, username, layout, has_monster):
    ...
    _draw_player_name(screen, username, player_rect.centerx, player_rect.top)


def _draw_narrative(screen, scene, username, panel, stype):
...
def play_game_view(screen, events, state):
    ...
    layout = _layout_zones(choice_count, ending)

    has_monster = bool(scene.get("monster"))

    if scene.get("show_player", True):
        _draw_player(screen, state, scene, game_state, username, layout, has_monster)

    if has_monster:
        _draw_monster_panel(
            screen, state, story_data, scene, layout["scene_bottom"], small_font
        )

    _draw_narrative(screen, scene, username, layout["panel"], stype)
    ...
    if ending:
        ...
        draw_button(screen, restart_rect, "Recommencer", accent=end_accent, hover_accent=end_hover)
        draw_button(screen, menu_rect, "Menu", accent=end_accent, hover_accent=end_hover)
        choice_rects = [("restart", restart_rect), ("menu", menu_rect)]
    else:
        ...
        for i, choice in enumerate(available):
            ...
            if rect.bottom > HEIGHT - 4:
                break
            btn_font = _font_for_button(choice["text"], rect.width, rect.height)
            draw_button(screen, rect, choice["text"], btn_font)
